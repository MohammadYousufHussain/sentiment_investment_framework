...
// "Compact" layout -- same data/scoring as HistoricalFactorScorecard.jsx
// (kept as-is, unmodified, for comparison via the toggle in ValuationTab).
// This variant only changes the layout: factor cards render in a 2x2 grid
// at roughly half the width instead of stacked full-width, with the
// per-metric stat cells reflowed from 4-across to 2x2 to stay readable at
// that narrower width. Set as the default view in ValuationTab.jsx.
...
// the trailing 12-quarter moving average ("normalized"). Hovering shows the
// exact period, actual value, and 12Q-average value via a crosshair + tooltip.
function MetricChart({ chart, metricKey }) {
  ...
  // Clamp to the visible range rather than extending the scale to fit -- see
  // axisBounds() above for why.
...
function StatCell({ label, value, tone, title }) {
  return (
    <div className="relative border border-hairline rounded-md bg-surface px-2 py-1.5 group/stat">
      ...
      {title && (
        <div
          className="pointer-events-none absolute left-0 bottom-full mb-1.5 w-48 rounded-md border border-hairline
  ...
  const devTitle =
    devPct == null
      ...
      : `${devPct >= 0 ? 'Above' : 'Below'} its own 12-quarter average -- ${metric.favorable ? 'favorable' : 'adverse'}` +
        ...
        (devLarge
          ? ' Large deviation -- for a genuinely cyclical company this can itself be a warning sign (check Stability).'
          : '')

  return (
    <div className="border-t border-hairline first:border-t-0 py-2.5">
      <div className="flex items-center justify-between gap-2 mb-1.5">
        <span className="text-[11px] font-medium text-ink-secondary">{metric.label}</span>
      ...
      <div className="grid grid-cols-2 gap-1.5 mt-2">
        ...
        <StatCell label="Normalized (12Q)" value={formatMetric(metricKey, metric.normalized)} />
        <StatCell
          label="Stability (CoV)"
          value={metric.stability != null ? metric.stability.toFixed(2) : '—'}
          title="Coefficient of Variation = stdev / mean over the trailing 12 quarters. Lower = more stable; higher = more cyclical."
        />
        <StatCell
          label="Deviation"
      ...
      {expanded && (
        <div className="mt-1.5 space-y-1">
          ...
          <p className="text-[10px] text-ink-muted">
            {metric.history_count} observations{metric.as_of ? ` · as of ${metric.as_of}` : ''}. z-score = deviation
            ÷ stability, direction-adjusted so positive always means favorable.
          </p>
        </div>
      )}
    </div>
...
function FactorDetail({ factorKey, factor, factorZ, weights, onWeightChange }) {
  ...
  return (
    <div className="border border-hairline rounded-lg bg-panel px-3 py-2.5 min-w-0">
      ...
      <p className="text-[10px] text-ink-muted leading-relaxed mb-1">{FACTOR_DESCRIPTIONS[factorKey]}</p>
      <div>
...
export default function HistoricalFactorScorecardBeta({ loading, error, data }) {
  ...
  return (
    <div className="border border-hairline rounded-lg bg-surface px-5 py-4">
      <div className="flex items-start justify-between mb-1">
        <div>
          <p className="text-sm font-semibold">Historical Factor Scorecard <span className="text-ink-muted font-normal">(Compact)</span></p>
          <p className="text-[11px] text-ink-muted mt-0.5">
            Same scoring as the Full view (z-score vs. this company's own trailing 12-quarter distribution;
            weight fields sum to 100% within each group, editing one rescales the rest) -- factor cards in a
            2x2 grid at roughly half width instead of stacked full-width.
          </p>
        </div>
